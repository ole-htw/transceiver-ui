...
class TransceiverUI(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        filter_frame, filter_body, _ = _make_side_bordered_group(gen_body, "Filter")
        ...
        self.bandwidth_entry.insert(0, "1e6")
        self.bandwidth_entry.grid(row=0, column=1, sticky="ew")
        self.bandwidth_entry.entry.bind(
            "<FocusOut>",
    ...
    def generate(self):
        try:
            ...
            if repeats > 1 and repeated_data is not None:
                repeat_base = self.file_entry.get()
                ...
                self._repeat_tx_file = None
                target_file = self.file_entry.get()
                self.tx_file.delete(0, tk.END)
            ...
            if zeros_data is not None:
                zeros_symbol_rate = repeated_symbol_rate or symbol_rate
